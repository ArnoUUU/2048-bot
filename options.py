...
def addsame(submatrix):
	gainedScore = submatrix[4]
	if submatrix[0] != None and submatrix[0]==submatrix[1]:      
		submatrix=[submatrix[0]+submatrix[1], submatrix[2],submatrix[3],None]
		gainedScore += submatrix[0]
	if submatrix[1] != None and submatrix[1]==submatrix[2]:
		submatrix=[submatrix[0],submatrix[1]+submatrix[2],submatrix[3],None]
		gainedScore += submatrix[1]
	if submatrix[2] != None and submatrix[2]==submatrix[3]:
		submatrix=[submatrix[0],submatrix[1], submatrix[2]+submatrix[3],None]
		gainedScore += submatrix[2]
	submatrix.append(gainedScore)
	return submatrix
#this is where all the computation happens when you moveleft. (the matrix will be ordered in a way )
def combine(matrix, score):
	submatrix0=[matrix[0],matrix[1],matrix[2],matrix[3],None]
	submatrix1=[matrix[4],matrix[5],matrix[6],matrix[7],None]
	submatrix2=[matrix[8],matrix[9],matrix[10],matrix[11],None]
	submatrix3=[matrix[12],matrix[13],matrix[14],matrix[15],None]
	...
	score += submatrix0[4]+submatrix1[4]+submatrix2[4]+submatrix3[4]
...
#this pushes all of the null strings to the end of the submatrix
def push(submatrix):
	nullcount = submatrix.count(None)
	for i in range(nullcount):
		submatrix.remove(None)
		submatrix.append(None)
	return submatrix
...
def moveleft(GameState):
	temp = combine(GameState.matrix,GameState.score)
	GameState.matrix = temp[0:16]
